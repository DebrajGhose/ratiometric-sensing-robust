...
fprintf(fid, 'variable kri = %g\n',kri); % receptor deactivation rate
fprintf(fid, '\n');
...
% Set up species
fprintf(fid, 'species Ram Rim Ga Gi A complex_Gi_Ra complex_Ga_Ri\n'); fprintf(fid, '\n');
...
for i = 1:N
    ...
    if i == 1 % For the first and last section we just need to add one boundary, as the other boundary is just the cell surface
        ...
        fprintf(fid, 'end_compartment\n');
        fprintf(fid, '\n');
        fprintf(fid, 'mol 1 A %g 0 0\n',cmpt_position);
        fprintf(fid, 'mol 1 A %g 0 %g\n',cmpt_position,mid_height);
        fprintf(fid, 'mol 1 A %g 0 %g\n',cmpt_position,-mid_height);
        fprintf(fid, 'mol 1 A %g 0 %g\n',cmpt_position,mid_height/2);
        fprintf(fid, 'mol 1 A %g 0 %g\n',cmpt_position,-mid_height/2);
    elseif i == N
...
fprintf(fid, 'reaction Gprotein_deactivation Ga(down) -> Gi(down) kri\n');
...
fprintf(fid, 'cmd I %d %d %d molpos Ram(all) %s\n', 100/dt, tstop/dt, samplingrate, xyz_name);
fprintf(fid, 'cmd I %d %d %d molpos Rim(all) %s\n', 100/dt, tstop/dt, samplingrate, xyz_name);
fprintf(fid, 'cmd I %d %d %d molpos Ga(all) %s\n', 100/dt, tstop/dt, samplingrate, xyz_name);
fprintf(fid, 'cmd I %d %d %d molpos Gi(all) %s\n', 100/dt, tstop/dt, samplingrate, xyz_name);
end
