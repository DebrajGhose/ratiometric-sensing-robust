...
kgi = 0.1687; % dervied from doing an exponential fit to activation curve
...
%Activate any G protein that is near an active receptor

if sum(Rprot(R_close_IDs{ii},4))>0 %look for any receptor that is active
Gprot(ii,4)=1; 
end
...
Gprot(Gprot(:,4)==1,4) = ~(rand(size(Gprot(Gprot(:,4)==1,4)))<dt*kgi);
